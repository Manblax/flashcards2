...
export default function UserMenu() {
  ...
  return (
    <div className="dropdown dropdown-end relative z-[100] shrink-0">
      ...
      <div
        tabIndex={0}
        className="dropdown-content right-0 top-full z-[100] mt-3 w-[280px] rounded-lg border border-[#262d55] bg-[#151a36] p-4 shadow-2xl shadow-black/35"
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <div className="text-base font-semibold leading-5 text-white truncate" title={user.username}>{user.username}</div>
            <div className="text-sm leading-5 text-neutral-content truncate" title={user.email}>{user.email}</div>
          </div>
        </div>

        <div className="my-4 h-px bg-[#252c52]"></div>

        {/* Меню */}
        <div className="space-y-1">
          <Link
            href="/settings"
            className="flex min-h-11 items-center gap-3 rounded-md px-3 py-2 text-[15px] font-medium leading-5 text-neutral-content transition-colors hover:bg-[#1d2447] hover:text-white"
          >
        ...
        <div>
          <button 
            ...
            className="flex min-h-11 w-full items-center gap-3 rounded-md px-3 py-2 text-left text-[15px] font-medium leading-5 text-neutral-content transition-colors hover:bg-[#1d2447] hover:text-white"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
